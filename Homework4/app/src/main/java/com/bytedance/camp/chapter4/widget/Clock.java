...
//        handler.sendEmptyMessage(1);
    ...
    private void drawTimeNumbers(Canvas canvas) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            float rectY = (float) (centerY + (radius * 0.6f + textHeight / 1.2f) * Math.sin(Math.toRadians(hourDegree)));
            float rectX = (float) (centerX + (radius * 0.6f + textWidth / 1.2f) * Math.cos(Math.toRadians(hourDegree)));
            float baselineY = (rectY + (bottom - top) / 2 - bottom);
