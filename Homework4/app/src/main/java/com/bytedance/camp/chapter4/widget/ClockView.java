...
public class ClockView extends View {
    ...
    private static final int HOUR_UNIT_DEGREE = 30;
    ...
    private static final int CLOCK_REFRESH = 1;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // handler.sendEmptyMessage(CLOCK_REFRESH);
        handler.sendMessageDelayed(Message.obtain(handler, CLOCK_REFRESH), 1000);
    }
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        float deg_hour = ( hour + (minute / 60f) ) * (float)HOUR_UNIT_DEGREE - 90;
        float deg_minute = minute * (float)UNIT_DEGREE - 90;
        float deg_second = second * (float)UNIT_DEGREE - 90;

        float hour_endX = (float)(centerX + radius * Math.cos(Math.toRadians(deg_hour))*HOUR_NEEDLE_LENGTH_RATIO);
        float hour_endY = (float)(centerY + radius * Math.sin(Math.toRadians(deg_hour))*HOUR_NEEDLE_LENGTH_RATIO);

        float minute_endX = (float)(centerX + radius * Math.cos(Math.toRadians(deg_minute))*MINUTE_NEEDLE_LENGTH_RATIO);
        float minute_endY = (float)(centerY + radius * Math.sin(Math.toRadians(deg_minute))*MINUTE_NEEDLE_LENGTH_RATIO);

        float second_endX = (float)(centerX + radius * Math.cos(Math.toRadians(deg_second))*SECOND_NEEDLE_LENGTH_RATIO);
        float second_endY = (float)(centerY + radius * Math.sin(Math.toRadians(deg_second))*SECOND_NEEDLE_LENGTH_RATIO);
        // Log.d("TAG", Float.toString(second_endX));
