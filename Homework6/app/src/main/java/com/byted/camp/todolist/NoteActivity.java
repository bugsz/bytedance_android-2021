...
public class NoteActivity extends AppCompatActivity {
    ...
    private boolean saveNote2Database(String content, Priority priority) {
        // TODO: 2021/7/19 8. 这里插入数据库（完成）
        if(database == null || content.isEmpty()) return false;
        ContentValues values = new ContentValues();
        values.put(TodoNote.COLUMN_NAME_CONTENT, content);
        values.put(TodoNote.COLUMN_NAME_DATE, System.currentTimeMillis());
        values.put(TodoNote.COLUMN_NAME_STATE, State.TODO.intValue);
        values.put(TodoNote.COLUMN_NAME_PRIORITY, priority.intValue);
